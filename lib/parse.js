...
module.exports = function(prg, arr) {
  ...
  // 1. Command opts:
  if(cmd) {
    lib.mandatoryOptsCheck(cmd.opts, res.opts, cmd.ignoreMandatoryOptCheckOpts);
  }

  // 2. Global opts:
  lib.mandatoryOptsCheck(prg.opts, res.gopts, prg.ignoreMandatoryOptCheckOpts);
  // except when help is requested, check for mandatory opts:
  // if(!(res.gopts.has('help') || (cmd && cmd.name === 'help'))) {
